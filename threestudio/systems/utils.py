...
def parse_optimizer_list(config, model):
    optim_list = []
    ...
    if config.name in ["FusedAdam"]:
        import apex
        for i in params:
            optim = getattr(apex.optimizers, config.name)(i, **config.args)
            optim_list.append(optim)
    elif config.name in ["Adan"]:
        from threestudio.systems import optimizers
        for i in params:
            optim = getattr(optimizers, config.name)(i, **config.args)
            optim_list.append(optim)
    else:
        for i in params:
            print(i["params"])
            optim = getattr(torch.optim, config.name)(i["params"], lr=i["lr"], **config.args)
            optim_list.append(optim)
    return optim_list
