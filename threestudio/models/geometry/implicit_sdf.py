...
class ImplicitSDF(BaseImplicitGeometry):
    ...
    def initialize_shape(self) -> None:
        ...
        if self.cfg.shape_init == "ellipsoid":
            ...
            mesh_base_path = self.cfg.shape_init[5:]
            sdf_network_init_path = os.path.join(mesh_base_path, 'head_sdf_network.pth')
            encoding_init_path = os.path.join(mesh_base_path, 'head_encoding.pth')
            assert os.path.exists(sdf_network_init_path)
            assert os.path.exists(encoding_init_path)
            self.sdf_network.load_state_dict(torch.load(sdf_network_init_path))
            self.encoding.load_state_dict(torch.load(encoding_init_path))
