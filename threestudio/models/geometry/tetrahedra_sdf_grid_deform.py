...
from threestudio.models.geometry.implicit_volume_sdf import ImplicitVolumeSDF
...
from threestudio.models.networks import get_encoding, get_mlp, DeformationNetwork, DeformationNetworkExp, DeformationNetworkConcat
from threestudio.utils.ops import scale_tensor
from threestudio.utils.typing import *


@threestudio.register("tetrahedra-sdf-grid-deform")
class TetrahedraSDFGridDeform(BaseExplicitGeometry):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.deformation_network_g = DeformationNetworkConcat(dim_in_film=self.encoding.n_output_dims, dim_out_film=3,
                                                        config=deformation_net_cfg)
        # self.deformation_network_g = DeformationNetworkExp(dim_in_film=self.encoding.n_output_dims, dim_out_film=3,
        #                                                 config=deformation_net_cfg)

        # self.deformation_network_g = DeformationNetwork(dim_in_film=self.encoding.n_output_dims, dim_out_film=3,
        #                                                 config=deformation_net_cfg)
        # self.deformation_network_g = DeformationNetwork(dim_in_film=3, dim_out_film=3,
        #                                                 config=deformation_net_cfg)
        # self.deformation_network_c = DeformationNetwork(dim_in_film=self.encoding.n_output_dims, dim_out_film=3,
        #                                                 config=deformation_net_cfg)
        self.can_mesh: Optional[Mesh] = None

    def initialize_shape(self) -> None:
        raise NotImplementedError

    def isosurface_deformation(self, exp):
        # return cached mesh if fix_geometry is True to save computation

        points_unscaled = scale_tensor(
            self.isosurface_helper.grid_vertices.to(self.bbox.device), self.isosurface_helper.points_range, self.bbox
        )
        ...
        for i in range(batch_size):
            exp_each = exp[i].unsqueeze(dim=0)
            deformation = self.deformation_network_g(enc, exp_each).reshape(*points.shape[:-1], 3)
            # deformation = self.deformation_network_g(self.deformation_face, exp_each).reshape(*points.shape[:-1], 3)
            mesh = self.isosurface_helper(self.sdf, deformation)
    ...
    def isosurface(self) -> Mesh:
        # return cached mesh if fix_geometry is True to save computation
        if self.cfg.fix_geometry and self.can_mesh is not None:
            return self.can_mesh
    ...
    def forward(
        self, points: Float[Tensor, "*N Di"], exp
    ) -> Dict[str, Float[Tensor, "..."]]:
        points_scale = contract_to_unisphere(points, self.bbox)  # points normalized to (0, 1)
        enc = self.encoding(points_scale.view(-1, self.cfg.n_input_dims))
        if exp is None:
            enc_can = enc
        else:
            deformation = self.deformation_network_c(enc, exp).reshape(*points.shape[:-1], 3)
            points_can = points + deformation
            points_scale_can = contract_to_unisphere(points_can, self.bbox)
            enc_can = self.encoding(points_scale_can.view(-1, self.cfg.n_input_dims))
            # enc_can = enc + deformation
        features = self.feature_network(enc_can).view(
            *points.shape[:-1], self.cfg.n_feature_dims
        )
        return {"features": features, "enc_can": enc_can, "exp": exp}
    ...
    def create_from(
        ...
        copy_net: bool = True,
        **kwargs,
    ) -> "TetrahedraSDFGridDeform":
        if isinstance(other, TetrahedraSDFGrid):
            instance = TetrahedraSDFGridDeform(cfg, **kwargs)
            ...
            return instance
        elif isinstance(other, ImplicitVolume) or isinstance(other, ImplicitVolumeSDF):
            instance = TetrahedraSDFGridDeform(cfg, **kwargs)
            ...
            raise TypeError(
                f"Cannot create {TetrahedraSDFGridDeform.__name__} from {other.__class__.__name__}"
            )
